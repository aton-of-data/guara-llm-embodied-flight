...
| D.9 | F´ v4.3.0 ships an uplink security stage: the `ComCcsdsSdls` subtopology inserts `CcsdsSdlsDeframer` → `SdlsSaRouter` → decryptor between the TC transfer-frame layer and Space Packet deframing; the deframer extracts a 16-bit security-association (SA) index and delegates to a decryptor selected per SA by a compile-time map. **The default decryptor is `Svc::Ccsds::ClearTextDecryptor`, which performs no authentication, no integrity checking and no decryption**, and the default SA map routes SA 0 to that pass-through port. Both SDDs say so in their own text. ⇒ an authenticated telecommand path is a *configuration* of upstream components plus a real decryptor, not new architecture; and a default F´ build is an unauthenticated command path, so the space side must declare the SA and the decryptor a run used (ADR 0015 rules 1 and 6). | `fprime@7d8f579:Svc/Subtopologies/ComCcsdsSdls/docs/sdd.md:8,11,20,23,50,81`; `fprime@7d8f579:Svc/Ccsds/ClearTextDecryptor/docs/sdd.md:4,32`; `fprime@7d8f579:Svc/Ccsds/CcsdsSdlsDeframer/docs/sdd.md:7-11,20-24` | HIGH |
| D.10 | `Svc::CmdDispatcher` decodes an opcode from an `Fw::Com` buffer, looks it up in a registration table (a red-black tree keyed by opcode, filled by each component's autogenerated `regCommands`), assigns a sequence number and dispatches. It performs **no authorization**: no requirement in its SDD conditions dispatch on the command's origin, and its only security-adjacent requirement is CD-005, dropping commands to avert a queue overflow. ⇒ nothing in the F´ command path decides *whether* a command is allowed; a gate has to be a component standing in the path (ADR 0015 rule 2). | `fprime@7d8f579:Svc/CmdDispatcher/docs/sdd.md:5,13-17,48,52` | HIGH |
| D.11 | `Svc::CmdSplitter` is the structural precedent for such a gate: a passive in-line component with `CmdBuff` as a `sync input array` of `Fw.Com`, `seqCmdStatus` as a `sync input array` of `Fw.CmdResponse`, output arrays mirroring the dispatcher's port arrays, and one configured state variable (`m_remoteBase`), which routes each command by comparing its opcode to a configured base and forwards every command response through. ⇒ `GuaraSpaceGateway` can take exactly these port shapes and reject rather than route (ADR 0015 rule 2). | `fprime@7d8f579:Svc/CmdSplitter/docs/sdd.md:5,28-32,38,42` | HIGH |
